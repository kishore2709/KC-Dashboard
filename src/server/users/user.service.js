...
async function updateDb(obj) {
  ...
  // console.log(rest);
  const updatePromise = await User.findByIdAndUpdate(id, { $set: rest }).lean();

  const changePasswordPromise = await changePassword(obj);
  return Promise.all([updatePromise, changePasswordPromise, rest]);
}
...
async function resetPassword(obj) {
  let { id, password, email, ...rest } = obj;
  ...
  if (result) {
    sendEmail(
      email,
      `KCDashboard: Thông báo mật khẩu mới`,
      `Mật khẩu mới của bạn là:${newPwd}
    Hãy đăng nhập vào hệ thống và đổi mật khẩu.
    `
    )
      .then(ok => {
        console.log('send ok');
